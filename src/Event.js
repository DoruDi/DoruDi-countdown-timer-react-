...
const Event = ({list, counter, index, removeItem}) => {
    ...
    const countdown = () => {
            ...
            if(remainingDay <= 0 && remainingHour <= 0 && remainingMinute <= 0 && remainingSecond == '00') {
                removeItem(index)
            }
            if (remainingDay < 0) {
                removeItem(index)
            }
            if(remainingDay <= 0 && remainingHour <= 0 && remainingMinute <= 0 && remainingSecond < 10) {

                    document.querySelectorAll('.specificEvent')[index].style.border = '5px solid red';
        ...
        return (
            <div className={'specificEvent'}>
                <div className="classList">
                    <span className="event-name"><p id="name"></p>{list.eventNameEl}</span>
                    <span><p>day</p><p id="day">{day}</p></span>
                    <span><p>hour</p><p id="hour">{hour}</p></span>
                    <span><p>minute</p><p id="minute">{minute}</p></span>
                    <span><p>second</p><p id="second">{second}</p></span>
                    <button className={'removeBtn'} onClick={() => removeItem(index)}><DeleteTwoToneIcon/></button>
                </div>
